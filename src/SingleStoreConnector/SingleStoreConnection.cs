...
#pragma warning disable CA1822 // Mark members as static
	...
	public sealed class SingleStoreConnection : DbConnection, ICloneable
	{
		...
		private SingleStoreConnection(string connectionString,
...
#pragma warning disable CA2012 // Safe because method completes synchronously
		...
		public new SingleStoreTransaction BeginTransaction() =>
			BeginTransactionAsync(IsolationLevel.Unspecified, default, IOBehavior.Synchronous, default).GetAwaiter()
				.GetResult();
		...
		/// <returns>A <see cref="SingleStoreTransaction"/> representing the new database transaction.</returns>
		/// <remarks>Transactions may not be nested.</remarks>
		public new SingleStoreTransaction BeginTransaction(IsolationLevel isolationLevel) =>
			BeginTransactionAsync(isolationLevel, default, IOBehavior.Synchronous, default).GetAwaiter().GetResult();
		...
		public SingleStoreTransaction BeginTransaction(IsolationLevel isolationLevel, bool isReadOnly) =>
			BeginTransactionAsync(isolationLevel, isReadOnly, IOBehavior.Synchronous, default).GetAwaiter().GetResult();
...
#pragma warning restore CA2012
 ...
 = default) => BeginTransactionAsync(isolationLevel, default, AsyncIOBehavior, cancellationToken);
	...
	protected override async ValueTask<DbTransaction> BeginDbTransactionAsync(IsolationLevel isolationLevel, CancellationToken cancellationToken) =>
		...
		/// <returns>A <see cref="Task{SingleStoreTransaction}"/> representing the new database transaction.</returns>
		/// <remarks>Transactions may not be nested.</remarks>
		public ValueTask<SingleStoreTransaction> BeginTransactionAsync(CancellationToken cancellationToken = default) =>
			BeginTransactionAsync(IsolationLevel.Unspecified, default, AsyncIOBehavior, cancellationToken);
		...
		public ValueTask<SingleStoreTransaction> BeginTransactionAsync(IsolationLevel isolationLevel,
			...
			BeginTransactionAsync(isolationLevel, default, AsyncIOBehavior, cancellationToken);
#endif
		...
		/// <returns>A <see cref="Task{SingleStoreTransaction}"/> representing the new database transaction.</returns>
		/// <remarks>Transactions may not be nested.</remarks>
		public ValueTask<SingleStoreTransaction> BeginTransactionAsync(IsolationLevel isolationLevel, bool isReadOnly,
			CancellationToken cancellationToken = default) =>
		...
		public override void EnlistTransaction(System.Transactions.Transaction? transaction)
		{
			...
			if (CurrentTransaction is not null)
				throw new InvalidOperationException(
					"Can't enlist in a Transaction when there is an active SingleStoreTransaction.");
			...
			if (GetInitializedConnectionSettings().UseXaTransactions)
			{
				if (hasEnlistedTransactions && !hasXaTransaction)
					throw new NotSupportedException(
						"Cannot start an XA transaction when there is an existing non-XA transaction.");
			}
			else if (hasEnlistedTransactions)
			{
				throw new NotSupportedException(
					"Multiple simultaneous connections or connections with different connection strings inside the same transaction are not supported when UseXaTransactions=False.");
			}
 ...
 default) => ChangeDatabaseAsync(AsyncIOBehavior, databaseName, cancellationToken);
		...
		public Task ChangeDatabaseAsync(string databaseName, CancellationToken cancellationToken = default) =>
			ChangeDatabaseAsync(AsyncIOBehavior, databaseName, cancellationToken);
#endif

		private async Task ChangeDatabaseAsync(IOBehavior ioBehavior, string databaseName,
...
#pragma warning restore CA2012
		public Task<bool> PingAsync(CancellationToken cancellationToken = default) =>
			PingAsync(SimpleAsyncIOBehavior, cancellationToken).AsTask();
		...
		internal async Task OpenAsync(IOBehavior? ioBehavior, CancellationToken cancellationToken)
		{
			...
			catch (Exception ex) when (activity is { IsAllDataRequested: true })
			{
				...
				if (m_connectionSettings?.ConnectionStringBuilder is { } connectionStringBuilder)
					activity.SetTag(ActivitySourceHelper.DatabaseConnectionStringTagName,
						connectionStringBuilder.GetConnectionString(connectionStringBuilder.PersistSecurityInfo));
		...
		/// </summary>
		public ConnectionInfo ServerThread => new(
		...
		public static Task ClearPoolAsync(SingleStoreConnection connection,
			CancellationToken cancellationToken = default) =>
			ClearPoolAsync(connection, connection.AsyncIOBehavior, cancellationToken);

		/// <summary>
		/// Clears all connection pools.
		/// </summary>
		public static void ClearAllPools() => ConnectionPool
		...
		public static Task ClearAllPoolsAsync(CancellationToken cancellationToken = default) =>
			ConnectionPool.ClearPoolsAsync(IOBehavior.Asynchronous, cancellationToken);
...
#pragma warning disable CA2012 // Safe because method completes synchronously
		...
		public override DataTable GetSchema() => GetSchemaProvider()
			.GetSchemaAsync(IOBehavior.Synchronous, "MetaDataCollections", default, default).GetAwaiter().GetResult();
		...
		/// <returns>A <see cref="DataTable"/> containing schema information.</returns>
		public override DataTable GetSchema(string collectionName) => GetSchemaProvider()
			.GetSchemaAsync(IOBehavior.Synchronous, collectionName, default, default).GetAwaiter().GetResult();
		...
		/// <returns>A <see cref="DataTable"/> containing schema information.</returns>
		public override DataTable GetSchema(string collectionName, string?[] restrictionValues) => GetSchemaProvider()
			.GetSchemaAsync(IOBehavior.Synchronous, collectionName, restrictionValues, default).GetAwaiter()
			.GetResult();
#pragma warning restore CA2012
...
#endif
			=> GetSchemaProvider().GetSchemaAsync(AsyncIOBehavior, "MetaDataCollections", default, cancellationToken)
				.AsTask();
 ...
 = default)
		...
		public Task<DataTable> GetSchemaAsync(string collectionName, string?[] restrictionValues,
			CancellationToken cancellationToken = default)
#endif
			=> GetSchemaProvider().GetSchemaAsync(AsyncIOBehavior, collectionName, restrictionValues, cancellationToken)
				.AsTask();
	...
	public override async ValueTask DisposeAsync()
		...
		public SingleStoreConnection CloneWith(string connectionString)
		{
			...
			return new SingleStoreConnection(this, dataSource, newConnectionString,
				m_hasBeenOpened && shouldCopyPassword && !currentBuilder.PersistSecurityInfo);
		}
		...
		internal ServerSession Session
		{
			get
			{
				...
				{
					throw new InvalidOperationException(
						$"Connection must be Open; current state is {State}, m_session is {m_session?.Id}");
				}
...
#if NET6_0_OR_GREATER
			var killQuerySql =
 string.Create(CultureInfo.InvariantCulture, $"KILL QUERY {command.Connection!.ServerThread.ConnectionId} {m_session.AggregatorId}");
#else
				var killQuerySql =
					FormattableString.Invariant(
						$"KILL QUERY {command.Connection!.ServerThread.ConnectionId} {m_session.AggregatorId}");
#endif
		...
		internal async Task<CachedProcedure?> GetCachedProcedure(string name, bool revalidateMissing,
			IOBehavior ioBehavior, CancellationToken cancellationToken)
		{
			...
			{
				cachedProcedure = await CachedProcedure.FillAsync(ioBehavior, this, normalized.Schema!,
					normalized.Component!, m_logger, cancellationToken).ConfigureAwait(false);
		...
		internal IOBehavior SimpleAsyncIOBehavior => (m_connectionSettings?.ForceSynchronous is true)
			? IOBehavior.Synchronous
			: IOBehavior.Asynchronous;
			...
			Activity? activity, IOBehavior? ioBehavior, CancellationToken cancellationToken)
		{
			...
			try
			{
				...
				else
				{
					...
					var loadBalancer =
						connectionSettings.LoadBalance == SingleStoreLoadBalance.Random &&
		...
		internal void SetState(ConnectionState newState)
		{
			if (m_connectionState != newState)
			{
				...
				var eventArgs =
					previousState == ConnectionState.Closed && newState == ConnectionState.Connecting
						?
						s_stateChangeClosedConnecting
						:
						previousState == ConnectionState.Connecting && newState == ConnectionState.Open
							? s_stateChangeConnectingOpen
							:
							previousState == ConnectionState.Open && newState == ConnectionState.Closed
								? s_stateChangeOpenClosed
								:
		...
		private SingleStoreConnection(SingleStoreConnection other, SingleStoreDataSource? dataSource,
			string connectionString, bool hasBeenOpened)
		...
		private static readonly StateChangeEventArgs s_stateChangeClosedConnecting =
			new(ConnectionState.Closed, ConnectionState.Connecting);

		private static readonly StateChangeEventArgs s_stateChangeConnectingOpen =
			new(ConnectionState.Connecting, ConnectionState.Open);
	...
	public readonly struct ConnectionInfo(int connectionId, int aggregatorId) : IEquatable<ConnectionInfo>
	{
		public int ConnectionId { get; } = connectionId;
		public int AggregatorId { get; } = aggregatorId;

		public bool Equals(ConnectionInfo other)
			=> ConnectionId == other.ConnectionId && AggregatorId == other.AggregatorId;
		...
		public override int GetHashCode() =>
			...
			System.HashCode.Combine(ConnectionId, AggregatorId);
#else
			(ConnectionId * 33) ^ AggregatorId;
#endif
