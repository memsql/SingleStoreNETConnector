...
internal sealed class StatementPreparer(string commandText, SingleStoreParameterCollection? parameters, StatementPreparerOptions options)
{
	...
	private SingleStoreParameter GetInputParameter(int index)
	{
		if (index >= (parameters?.Count ?? 0))
			throw new SingleStoreException($"Parameter index {index} is invalid when only {parameters?.Count ?? 0} parameter{(parameters?.Count == 1 ? " is" : "s are")} defined.");
