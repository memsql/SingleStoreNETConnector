...
Set [`SourceOrdinal`](./SingleStoreBulkCopyColumnMapping/SourceOrdinal.md) to the index of the source column to map. Set [`DestinationColumn`](./SingleStoreBulkCopyColumnMapping/DestinationColumn.md) to either the name of a column in the destination table, or the name of a user-defined variable. If a user-defined variable, you can use [`Expression`](./SingleStoreBulkCopyColumnMapping/Expression.md) to specify a SingleStore expression that assigns its value to destination column.
...
| [SourceOrdinal](SingleStoreBulkCopyColumnMapping/SourceOrdinal.md) { get; set; } | The ordinal position of the source column to map from. |
