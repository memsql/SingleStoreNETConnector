# ISingleStoreConnectorLogger.Log method
...
This method may be called from multiple threads and must be thread-safe. This method may be called even if [`IsEnabled`](./IsEnabled.md) would return `false` for *level*; the implementation must check if logging is enabled for that level.
