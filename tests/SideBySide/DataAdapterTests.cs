...
");
	...
	{
		var command = new SingleStoreCommand(sql, m_connection);
		foreach (var parameterName in parameterNames.Split(','))
			command.Parameters.Add(new SingleStoreParameter(parameterName, SingleStoreDbType.Int32));
		var parser = new SingleStoreDataAdapter.InsertSqlParser(command);
		parser.Parse(sql);
	...
	{
		var batch = new SingleStoreBatch(m_connection);
		for (var i = 0; i < commandCount; i++)
		{
			var batchCommand = new SingleStoreBatchCommand(insertSql);
			for (var j = 0; j < parameterCount; j++)
			{
				batchCommand.Parameters.Add(new SingleStoreParameter($"@param{j}", SingleStoreDbType.Int32));
			}
			batch.BatchCommands.Add(batchCommand);
		}

		var command = SingleStoreDataAdapter.TryConvertToCommand(batch);
		if (expected is null)
		{
			Assert.Null(command);
		}
	...
	public void ConvertBatchToCommandParameters()
	{
		...
		var batch = new SingleStoreBatch(m_connection)
		{
			BatchCommands =
			{
				new SingleStoreBatchCommand(insertSql)
				{
					Parameters =
					{
						new SingleStoreParameter("@c1", 1),
						new SingleStoreParameter("@c2", 2),
					},
				},
				new SingleStoreBatchCommand(insertSql)
				{
					Parameters =
					{
						new SingleStoreParameter("@c1", 3),
						new SingleStoreParameter("@c2", 4),
					},
				},
			},
		};
