...
public class ParameterTests
{
	...
	public void VectorParameter_WithDimensionValidation_CorrectDimensions_Succeeds()
	{
		using var connection = new SingleStoreConnection(AppConfig.ConnectionString);
		connection.Open();

		using var cmd = connection.CreateCommand();
		cmd.CommandText = "SELECT @vec";

		var vector = new float[] { 1.0f, 2.0f, 3.0f, 4.0f };
		var param = new SingleStoreParameter
		{
			ParameterName = "@vec",
			SingleStoreDbType = SingleStoreDbType.Vector,
			Value = vector,
			VectorDimensions = 4,
			VectorElementTypeName = "F32",
		};
	...
	[SkippableFact(ServerFeatures.ExtendedDataTypes)]
	public void VectorParameter_WithDimensionValidation_WrongDimensions_ThrowsArgumentException()
	{
		using var connection = new SingleStoreConnection(AppConfig.ConnectionString);
		connection.Open();
		...
		var vector = new float[] { 1.0f, 2.0f }; // 2 dimensions
		var param = new SingleStoreParameter
		{
			ParameterName = "@vec",
			SingleStoreDbType = SingleStoreDbType.Vector,
			Value = vector,
			VectorDimensions = 4, // Expects 4 dimensions
			VectorElementTypeName = "F32",
		};
		cmd.Parameters.Add(param);

		var ex = Assert.Throws<ArgumentException>(() => cmd.ExecuteScalar());
	...
	[SkippableFact(ServerFeatures.ExtendedDataTypes)]
	public void VectorParameter_WithoutDimensionValidation_AnyDimensions_Succeeds()
	{
		using var connection = new SingleStoreConnection(AppConfig.ConnectionString);
		connection.Open();
		...
		var param = new SingleStoreParameter
		{
			...
			SingleStoreDbType = SingleStoreDbType.Vector,
			Value = vector,
			//// No VectorDimensions set - validation is skipped
		};
		cmd.Parameters.Add(param);
	...
	[SkippableFact(ServerFeatures.ExtendedDataTypes)]
	public void BsonParameter_ValidBson_Succeeds()
	{
		using var connection = new SingleStoreConnection(AppConfig.ConnectionString);
		connection.Open();

		using var cmd = connection.CreateCommand();
		cmd.CommandText = "SELECT @bson";

		// Valid minimal BSON document
	...
	[SkippableFact(ServerFeatures.ExtendedDataTypes)]
	public void BsonParameter_InvalidBson_ThrowsFormatException()
	{
		using var connection = new SingleStoreConnection(AppConfig.ConnectionString);
		connection.Open();
		...
		var ex = Assert.Throws<FormatException>(() => cmd.ExecuteScalar());
		Assert.Contains("BSON", ex.Message);
	}
