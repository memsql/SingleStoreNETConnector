...
";
	...
	public void GetColumnSchemaForTableAlias()
	{
		...
		cmd.CommandText = """
	                      ...
	                      create table column_schema_table2(id int not null primary key, table1_id1 int not null,
		...
		Assert.Equal("t2", ((SingleStoreDbColumn) schema[0]).TableName);
		Assert.Equal("t1a", ((SingleStoreDbColumn) schema[1]).TableName);
		Assert.Equal("t1b", ((SingleStoreDbColumn) schema[2]).TableName);
	}
#endif
...
select mysql_query_attribute_string('attr2') as attribute, @param2 as parameter;", connection);
	...
	public void GetBytesByOrdinal()
	{
		using var connection = new SingleStoreConnection(AppConfig.ConnectionString);
		connection.Open();
		using var cmd = new SingleStoreCommand("select X'0123456789ABCDEF';", connection);
		using var reader = cmd.ExecuteReader();
		Assert.True(reader.Read());
	...
	public void GetBytesByName()
	{
		using var connection = new SingleStoreConnection(AppConfig.ConnectionString);
		connection.Open();
		using var cmd = new SingleStoreCommand("select X'0123456789ABCDEF' as tmp;", connection);
		using var reader = cmd.ExecuteReader();
		Assert.True(reader.Read());
