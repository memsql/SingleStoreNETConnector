...
The tests require a MySQL server. The simplest way to run one is with [Docker](https://www.docker.com/community-edition):
    ...
    docker run -d --rm --name mysqlconnector -e MYSQL_ROOT_PASSWORD=pass -p 3306:3306 mysql:8.2 --max-allowed-packet=96M --character-set-server=utf8mb4 --log-bin-trust-function-creators=1 --local-infile=1 --max-connections=250
